...
if [ $# -ne 4 ]; then
    echo "Usage: ./mut_qm_sel.sh <residue_number> <residue> <topology> <qm_selection>"
    exit 1
fi
...
if [ "$res_name" != "GLY" ]; then
        sc_atoms_wt=$(cpptraj -p ../"$topology" --mask ":"$res_num"&!(@CA,C,O,N,H1,H2,H3,H,HA,HA2,HA3)" | tail -n +2 |  awk '{print $2}')
fi
...
bb_atoms_mut=$(cpptraj -p "$res_type"_"$res_num".prmtop --mask :"$res_num"@CA,C,O,N,H1,H2,H3,H,HA,HA2,HA3 | tail -n +2 |  awk '{print $2}')
if [ "$res_type" != "GLY" ]; then
        sc_atoms_mut=$(cpptraj -p "$res_type"_"$res_num".prmtop --mask ":"$res_num"&!(@CA,C,O,N,H1,H2,H3,H,HA,HA2,HA3)" | tail -n +2 |  awk '{print $2}')
fi
